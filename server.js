...
// Function to get DeepSeek embedding
async function getEmbedding(text) {
  const url = `${process.env.EMBED_BASE_URL}/embeddings`;
...
// Normalize vector (optional for similarity calculation)
function norm(v) {
  return Math.sqrt(v.reduce((s, x) => s + x * x, 0)) || 1;
}

// Dot product of two vectors (used for similarity)
function dot(a, b) {
  let s = 0;
...
// Precompute embeddings for all cards (called once when the app starts)
async function buildIndex() {
  CARD_VECS = [];
  for (const c of CARDS) {
    const text = `${c.title} ${c.content} ${(c.synonyms || []).join(' ')}`;
    const vec = await getEmbedding(text);
    CARD_VECS.push({ id: c.id, case_id: c.case_id, vec, norm: norm(vec) });
  }
  console.log(`Indexed ${CARD_VECS.length} cards.`);
}

// Endpoint to initialize case (e.g., show chief complaint)
...
// Ask endpoint: search for matching cards based on the student's question
app.post('/ask', async (req, res) => {
  try {
    ...
    const threshold = Number(process.env.SIM_THRESHOLD || 0.4);
    const K = Number(process.env.TOP_K || 1);

    // Filter candidates: same case_id & not already revealed
    const candidates = CARD_VECS.filter(v => v.case_id === String(case_id) && !revealed_ids.includes(v.id));

    // Compute similarity score for each card
    const scored = candidates.map(v => ({
      id: v.id,
      score: dot(qVec, v.vec) / (qNorm * v.norm),
    })).sort((a, b) => b.score - a.score);

    // Get Top-K hits, above the similarity threshold
    const hits = [];
